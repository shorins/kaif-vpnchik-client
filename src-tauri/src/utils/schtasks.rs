...
const TASK_NAME_USER: &str = "kaif vpnchik";
const TASK_NAME_ADMIN: &str = "kaif vpnchik (Admin)";
...
async fn cleanup_legacy_shortcuts() -> Result<()> {
    ...
    let kaif_shortcut = startup_dir.join("kaif vpnchik.lnk");

    old_shortcut.remove_if_exists().await?;
    new_shortcut.remove_if_exists().await?;
    kaif_shortcut.remove_if_exists().await?;
    Ok(())
}
